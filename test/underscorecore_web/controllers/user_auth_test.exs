defmodule UnderscorecoreWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, UnderscorecoreWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert App.get_user_by_session_token(token)
    end
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      user_token = App.generate_user_session_token(user)
      ...
      assert redirected_to(conn) == "/"
      refute App.get_user_by_session_token(user_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "users_sessions:abcdef-token"
      UnderscorecoreWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_user/2" do
    ...
    test "does not authenticate if data is missing", %{conn: conn, user: user} do
      _ = App.generate_user_session_token(user)
