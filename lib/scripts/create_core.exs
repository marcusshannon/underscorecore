defmodule CreateCoreScript do
  ...
  alias Underscorecore.Models.{CoreAlbum, Core}

  def fetch_collage(collage_id) do
    Finch.build(:get, "https://redacted.ch/ajax.php?action=collage&id=#{collage_id}", [
      {"Authorization", "33ea1089.f686fdc94559acdde59caa7e9ec4889b"}
    ])
  ...
  def map_to_search_term(torrent_groups) do
    Enum.map(torrent_groups, fn torrent_group ->
      ...
      artist =
        (torrent_group["musicInfo"]["artists"] |> List.first())["name"] |> HtmlEntities.decode()
  ...
  def main(collage_id) do
    ...
    unless Underscorecore.Repo.exists?(from c in "cores", where: c.name == ^core_name) do
      Underscorecore.Repo.insert(%Core{name: core_name})
    end

    core = Underscorecore.Repo.get_by!(Core, name: core_name)

    Enum.each(search_terms, fn search_term ->
      Process.sleep(2000)

      try do
        search_results = App.search(search_term)

        unless length(search_results) == 0 do
          {artist, album} = List.first(search_results)

          Underscorecore.Repo.insert(
            %CoreAlbum{album_id: album.id, core_id: core.id},
            on_conflict: :nothing
          )
        end
      rescue
